...
public class DoubleExpressionTest extends ExpressionTest {
    ...
    private void generated() {
        ...
        final Const ce1 = new Const(1.1E10);
        final Const ce2 = new Const(2.3E12);
        testExpression("((1.1E10 * x) * (1.1E10 / 2.3E12))", "1.1E10 * x * (1.1E10 / 2.3E12)", new Multiply(new Multiply(ce1, vx), new Divide(ce1, ce2)), x -> 1.1E10 * x * (1.1E10 / 2.3E12));
        testExpression("((1.1E10 * x) / (x + x))", "1.1E10 * x / (x + x)", new Divide(new Multiply(ce1, vx), new Add(vx, vx)), x -> 1.1E10 * x / (x + x));
        testExpression("(((x - x) / 2.3E12) + 2.3E12)", "(x - x) / 2.3E12 + 2.3E12", new Add(new Divide(new Subtract(vx, vx), ce2), ce2), x -> (x - x) / 2.3E12 + 2.3E12);
        testExpression("((x / (x - x)) - 1.1E10)", "x / (x - x) - 1.1E10", new Subtract(new Divide(vx, new Subtract(vx, vx)), ce1), x -> x / (x - x) - 1.1E10);
        testExpression("((2.3E12 - (1.1E10 * x)) * 1.1E10)", "(2.3E12 - 1.1E10 * x) * 1.1E10", new Multiply(new Subtract(ce2, new Multiply(ce1, vx)), ce1), x -> (2.3E12 - 1.1E10 * x) * 1.1E10);
        testExpression("((x / (x - x)) / x)", "x / (x - x) / x", new Divide(new Divide(vx, new Subtract(vx, vx)), vx), x -> x / (x - x) / x);
    }
